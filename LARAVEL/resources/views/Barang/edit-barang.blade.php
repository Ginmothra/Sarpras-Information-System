...
<body class="relative h-screen overflow-hidden">
    ...
    <div class="relative z-10 flex items-center justify-center h-full">
        <form action="{{ route('update.barang', $barang->id) }}" method="POST"
            ...
            <h1 style="font-family: 'Nunito', serif; font-weight: 900;" class="text-xl font-bold mb-4 tracking-wider">
                EDIT BARANG</h1>
            <input class="block mb-4 p-2.5 outline-none rounded-[14px] w-70 bg-white drop-shadow-lg/40" type="text"
                name="nama" value="{{ $barang->nama }}" required>

            <input class="block mb-4 p-2.5 outline-none rounded-[14px] w-70 bg-white drop-shadow-lg/40" type="text"
                name="kode_barang" value="{{ $barang->kode_barang }}" required>
            ...
            <button type="submit" class="mt-6 bg-[#377DC2] drop-shadow-lg/40 text-white px-4 py-2 rounded w-50">
                <p style="font-family: 'Nunito', serif; font-weight: 400;">Edit Barang</p>
            </button>
        </form>
    </div>
</body>
